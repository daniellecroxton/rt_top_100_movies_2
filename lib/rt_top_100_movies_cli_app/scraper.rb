...
class RtTop100MoviesCliApp::Scraper
  ...
  def self.scrape_movie(details_url)
    ...
    details_page.css('#mainColumn').each do | detail |
      ...
      movie_details[:critic_consensus] = detail.css(".critic_consensus.tomato-info.noSpacing.superPageFontColor").text.strip.sub("Critic Consensus:\n", "")
      movie_details[:rating] = detail.css("div.panel-body ul.content-meta li .meta-value").first.text.strip
      movie_details[:release_date] = detail.css("div.panel-body ul.content-meta li .meta-value")[4].text.strip.gsub(/\s+/, "")
      movie_details[:genre] = detail.css("div.panel-body ul.content-meta li .meta-value")[1].text.strip.gsub(/\s+/, "")
